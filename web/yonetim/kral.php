<?php
    ...
    if ($_POST) {
        if (isset($_POST["aciklama"])) {
            ...
            $sorgu = $baglan->query("UPDATE kral SET aciklama = '$aciklama'");
        }

        if (isset($_POST["aciklama2"])) {
            $aciklama = $_POST["aciklama2"];
            $sorgu = $baglan->query("UPDATE kral SET aciklama2 = '$aciklama'");
        }

        if (isset($_POST["aciklama3"])) {
            $aciklama = $_POST["aciklama3"];
            $sorgu = $baglan->query("UPDATE kral SET aciklama3 = '$aciklama'");
        }
    }

    $sorgu = $baglan->query("select * from kral");
